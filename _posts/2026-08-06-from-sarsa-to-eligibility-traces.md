...
<span style="color:#d1242f"><strong>这条链路一直在解决同一个问题：怎样根据一条条真实轨迹，更准确、更高效地估计当前状态或动作的长期价值。</strong></span>
...
<span style="color:#d1242f"><strong>未来尚未发生时，$G_t$ 是一个随机变量；轨迹发生后，它才成为可以计算的确定结果。算法中的 $V$ 和 $Q$ 才是价值估计。</strong></span>
...
<span style="color:#d1242f"><strong>Sarsa 真正更新的是 $Q(S_t,A_t)$；$Q(S_{t+1},A_{t+1})$ 在当前这一步主要用于构造 target。</strong></span>
...
这里的 $G_{t:t+n}$ 可以理解为：**从时间 $t$ 开始，沿同一条实际轨迹向前走 $n$ 步，在时间 $t+n$ 处停止展开并进行 bootstrap 所得到的 n-step return。**

其中：

- $t$：这次要更新的状态—动作对所在的起始时间；
- $t+n$：向未来实际观察 $n$ 步后的 bootstrap 边界；
- $R_{t+1},\ldots,R_{t+n}$：这 $n$ 步中真实观察到的奖励；
- $Q(S_{t+n},A_{t+n})$：从第 $n$ 步以后剩余未来价值的当前估计。

<span style="color:#d1242f"><strong>因此，$G_{t:t+n}$ 不是 $n$ 条轨迹的平均，而是同一条实际轨迹上的连续 $n$ 步。</strong></span>
...
<span style="color:#d1242f"><strong>$\lambda$-return 混合的是不同长度的 n-step return，而不是把不同轨迹直接混在一起。</strong></span>
...
<p class="figure-caption">图 2：Gridworld 中不同算法对路径上动作价值的更新范围对比。图源：Richard S. Sutton、Andrew G. Barto，《Reinforcement Learning: An Introduction》（第 2 版），Example 12.1。</p>
...
这里的 $\mathbb I$ 是 **indicator function（指示函数）**：
...
> **当前时间 $t$ 有没有访问状态—动作对 $(s,a)$？**

如果当前访问了，就加 1；没有访问，就加 0。

<span style="color:#d1242f"><strong>需要注意：$\mathbb I$ 本身不是访问次数。真正把过去多次访问的影响累积起来的是资格迹 $Z_t(s,a)$。</strong></span>

因为以前的资格会按 $\gamma\lambda$ 保留下来，所以 $Z_t(s,a)$ 可以理解成一种**带时间衰减的历史访问痕迹**：

- 刚刚访问过：资格较大；
- 距离当前越来越久：资格逐渐衰减；
- 如果之后再次访问同一个 $(s,a)$，又会在原有资格上增加新的访问贡献。
...
<span style="color:#d1242f"><strong>资格迹传播的是 TD error，而不是直接传播奖励，也不是把后面的 Q 值复制到前面的 Q。</strong></span>
